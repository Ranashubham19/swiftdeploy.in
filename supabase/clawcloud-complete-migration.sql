...
create table if not exists public.connected_accounts (
  ...
  provider text not null check (
    provider in ('gmail', 'google_calendar', 'google_drive', 'whatsapp', 'telegram', 'slack')
  ),
  access_token text,
  refresh_token text,
...
create table if not exists public.whatsapp_contacts (
  ...
  notify_name text,
  verified_name text,
  source text not null default 'session' check (
    source in ('session', 'history', 'message')
  ),
...
create table if not exists public.whatsapp_messages (
  ...
  remote_phone text,
  contact_name text,
  chat_type text not null default 'direct' check (
    chat_type in ('direct', 'group', 'self', 'broadcast', 'unknown')
  ),
