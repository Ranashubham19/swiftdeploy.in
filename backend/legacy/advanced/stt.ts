...
type SttProvider = OpenAICompatibleSttProvider | AssemblyAiSttProvider;

const truthy = (value: string): boolean => /^(1|true|yes|on)$/i.test(value);
...
const normalizeSttEndpoint = (raw: string): string => {
...
const readTextFromTranscriptionPayload = (payload: any): string => {
  const candidates = [
    payload?.text,
    payload?.transcript,
    payload?.result?.text,
...
const buildSttProviders = (): SttProvider[] => {
  const providers: SttProvider[] = [];

  const assemblyKey = String(
    process.env.ASSEMBLYAI_API_KEY || process.env.STT_ASSEMBLYAI_API_KEY || "",
  ).trim();
  const assemblyEnabledRaw = String(
    process.env.ASSEMBLYAI_STT_ENABLED || process.env.STT_USE_ASSEMBLYAI || "",
  ).trim();
  const assemblyEnabled = assemblyEnabledRaw ? truthy(assemblyEnabledRaw) : !!assemblyKey;
  if (assemblyEnabled && assemblyKey) {
    const assemblyBaseUrl = normalizeAssemblyAiBaseUrl(
      String(process.env.ASSEMBLYAI_BASE_URL || "https://api.assemblyai.com/v2").trim(),
    );
    const assemblySpeechModel = String(
      process.env.ASSEMBLYAI_SPEECH_MODEL || process.env.ASSEMBLYAI_MODEL || "universal-2",
    ).trim();
    const assemblyPollIntervalMs = clamp(
      Number(process.env.ASSEMBLYAI_POLL_INTERVAL_MS || 1800),
      500,
      5000,
    );
    providers.push({
  ...
  const sttBase = normalizeSttEndpoint(
    String(process.env.STT_BASE_URL || process.env.STT_ENDPOINT || "").trim(),
  );
  ...
  const nvidiaToggleRaw = String(
    process.env.NVIDIA_STT_ENABLED || process.env.STT_USE_NVIDIA || "",
  ).trim();
  ...
  const nvidiaBase = normalizeSttEndpoint(
    String(
      process.env.NVIDIA_STT_BASE_URL
  ...
  const nvidiaModel = String(
    process.env.NVIDIA_STT_MODEL || process.env.STT_NVIDIA_MODEL || "whisper-1",
  ).trim();
  ...
  const deduped: SttProvider[] = [];
  for (const provider of providers) {
    const key =
      provider.kind === "assemblyai"
...
const transcribeWithOpenAICompatibleProvider = async (
  mediaBlob: Blob,
...
const transcribeWithAssemblyAiProvider = async (
  mediaInput: { fileUrl: string; mediaBlob?: Blob | null },
  provider: AssemblyAiSttProvider,
  opts?: { language?: string; prompt?: string; timeoutMs?: number },
): Promise<string | null> => {
  const totalTimeoutMs = Math.max(
    12000,
    opts?.timeoutMs ?? Number(process.env.ASSEMBLYAI_STT_TIMEOUT_MS || 45000),
  );
  const deadline = Date.now() + totalTimeoutMs;
  const uploadToAssemblyAi = async (mediaBlob: Blob): Promise<string | null> => {
    const uploadController = new AbortController();
    const uploadTimeout = setTimeout(
      () => uploadController.abort(),
      Math.max(3000, Math.min(totalTimeoutMs, 20000)),
    );
    try {
  ...
  if (!transcriptAudioUrl) return null;
  ...
  const submitTimeout = setTimeout(
    ...
    Math.max(3000, Math.min(totalTimeoutMs, 15000)),
  );
  try {
    ...
    latestPayload = (await submitResponse.json().catch(() => ({}))) as any;
    ...
    if (latestStatus === "error" || !transcriptId) return null;
  } catch {
    return null;
  } finally {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      const pollResponse = await fetch(
        `${provider.baseUrl}/transcript/${encodeURIComponent(transcriptId)}`,
        {
          method: "GET",
          headers: {
            authorization: provider.apiKey,
      ...
      latestPayload = (await pollResponse.json().catch(() => ({}))) as any;
      latestStatus = String(latestPayload?.status || "").toLowerCase();
      if (latestStatus === "completed") {
        const text = readTextFromTranscriptionPayload(latestPayload);
        return text || null;
...
export const transcribeTelegramMediaFromUrl = async (
  fileUrl: string,
  opts?: { language?: string; prompt?: string; timeoutMs?: number },
): Promise<string | null> => {
  ...
  for (const provider of providers) {
    if (provider.kind === "assemblyai") {
      ...
      const text = await transcribeWithAssemblyAiProvider(
        { fileUrl: url, mediaBlob },
        provider,
        opts,
      );
      if (text) return text;
