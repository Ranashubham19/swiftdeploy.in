export type ContextReferenceTarget = "question" | "answer" | "pair";

export type ParsedContextReference = {
  isReference: boolean;
  target: ContextReferenceTarget;
  ordinal: number | null;
  latest: boolean;
...
export const parseContextReference = (input: string): ParsedContextReference => {
  ...
  const ordMatch =
    text.match(/\b(\d+)(?:st|nd|rd|th)?\s+(question|answer|response|reply|point|step)\b/) ||
  ...
  const contextWords =
    /\b(context|conversation|message|chat|thread|history)\b/.test(text)
