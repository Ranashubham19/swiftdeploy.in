  ...
  private async requestStream(
    ...
    return this.withRetry(async () => {
      ...
      if (contentType.includes("application/json")) {
        const raw = await response.text();
        const parsed = raw ? (JSON.parse(raw) as ChatCompletionApiResponse) : null;
      ...
      const processEventChunk = (chunk: string): void => {
        const lines = chunk
          .split(/\r?\n/)
          .map((line) => line.trim())
  ...
  private async withRetry<T>(
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt += 1) {
      ...
      try {
        ...
        const delay =
          Math.min(2400, this.retryBaseDelayMs * 2 ** attempt) + Math.random() * 90;
        logger.warn(
