...
export const LOCKED_NVIDIA_MODEL_ID = fromEnv(
  "NVIDIA_MODEL",
  fromEnv("DEFAULT_MODEL", fromEnv("OPENROUTER_MODEL", "meta/llama-3.3-70b-instruct")),
);
...
const zeroCostOpenRouterModelIdSet = new Set<string>(
  ZERO_COST_OPENROUTER_MODEL_IDS.map((id) => id.toLowerCase()),
);

export const isFreeOnlyApprovedModelId = (modelId: string | null | undefined): boolean => {
...
const curatedFreeModelPoolByRole: Record<FreeModelRole, readonly string[]> = {
...
export const CURATED_FREE_MODEL_POOLS: Record<FreeModelRole, string[]> = Object.fromEntries(
  Object.entries(curatedFreeModelPoolByRole).map(([role, ids]) => {
    const unique = Array.from(new Set(ids.map((id) => id.trim()).filter(Boolean)));
    return [role, unique];
  }),
) as Record<FreeModelRole, string[]>;

const curatedStrongFreeModelIdSet = new Set<string>(
  Object.values(CURATED_FREE_MODEL_POOLS)
    .flat()
    .map((id) => String(id || "").trim().toLowerCase())
    .filter(Boolean),
);

export const isCuratedStrongFreeModelId = (modelId: string | null | undefined): boolean => {
  const normalized = String(modelId || "").trim().toLowerCase();
  if (!normalized) return false;
  return curatedStrongFreeModelIdSet.has(normalized);
};

export const getCuratedFreeModelPool = (
  role: FreeModelRole,
...
const modelIdWithFreeOnly = (
  _envName: string,
  _fallback: string,
  _options?: { allowNonFree?: boolean },
): string => {
...
export const FALLBACK_MODEL_ID = modelIdWithFreeOnly(
  "FALLBACK_MODEL",
  LOCKED_NVIDIA_MODEL_ID,
);
