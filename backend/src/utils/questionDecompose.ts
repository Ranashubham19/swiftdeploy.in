...
export const decomposeQuestionParts = (input: string, maxParts = 6): DecomposedQuestion => {
...
export const buildQuestionBreakdownInstruction = (input: string): string => {
  const decomposed = decomposeQuestionParts(input);
  if (!decomposed.isMultiPart) return "";
