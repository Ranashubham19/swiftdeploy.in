...
const codeStartSignalPattern =
  /\b(def\s+[A-Za-z_][A-Za-z0-9_]*\s*\(|class\s+[A-Za-z_][A-Za-z0-9_]*|function\s+[A-Za-z_][A-Za-z0-9_]*\s*\(|const\s+[A-Za-z_][A-Za-z0-9_]*\s*=|let\s+[A-Za-z_][A-Za-z0-9_]*\s*=|var\s+[A-Za-z_][A-Za-z0-9_]*\s*=|#include\b|public class\b|using\s+namespace\b|using\s+std::|fn\s+[A-Za-z_][A-Za-z0-9_]*\s*\(|SELECT\b|INSERT\b|UPDATE\b|DELETE\b|CREATE TABLE\b|if\s*\([^)]*\)|for\s*\([^)]*;[^)]*;[^)]*\)|while\s*\([^)]*\)|switch\s*\([^)]*\)|<!DOCTYPE html|<html)/i;
const codeNarrativeTailPattern =
  /\s+(?:This approach|The algorithm works|Time complexity|Space complexity|Complexity|Explanation|In summary|You can|This solution)\b[\s\S]*$/i;
  ...
  for (let i = 0; i < source.length; i += 1) {
    ...
    if (ch === ";" && parenDepth === 0) {
      ...
      const shouldStayInline =
        afterToken === "else" || afterToken === "while" || afterToken === "catch" || afterToken === "finally";
...
const reflowCompressedCode = (value: string, languageHint: string): string => {
  ...
  const looksPython =
    ...
    /\b(def |class |import |from |if |elif |else:|for |while |try:|except |with |return )/.test(
      code,
    );
  ...
  if (looksPython) {
    const pythonFormatted = code
      ...
      .replace(/;\s*/g, ";\n")
      .replace(
        /\s+(?=def |class |import |from |if |elif |else:|for |while |try:|except |with |return |pass|break|continue)/g,
        "\n",
      )
      .replace(
  ...
  const genericFormatted = insertLineBreaksAfterStatementSemicolons(code)
    ...
    .replace(/\s*\}/g, "\n}\n")
    .replace(
      /\s+(?=function |const |let |var |if |else |for |while |return |class |import |from |public |private |protected |async )/g,
      "\n",
    )
    .replace(/\n{3,}/g, "\n\n")
    .trim();
...
const cleanCode = (value: string, languageHint = ""): string => {
...
const stripCodeNarrativeTail = (value: string): string =>
  String(value || "").replace(codeNarrativeTailPattern, "").trim();

const looksCodeDense = (value: string): boolean => {
  const text = String(value || "").trim();
  if (!text) return false;

  const keywordHits = (
    text.match(
      /\b(def\s+[A-Za-z_][A-Za-z0-9_]*\s*\(|class\s+[A-Za-z_][A-Za-z0-9_]*|function\s+[A-Za-z_][A-Za-z0-9_]*\s*\(|const\s+[A-Za-z_][A-Za-z0-9_]*\s*=|let\s+[A-Za-z_][A-Za-z0-9_]*\s*=|var\s+[A-Za-z_][A-Za-z0-9_]*\s*=|import |from |return\b[^\n;]*;|if\s*\([^)]*\)|elif\b[^\n]*:|else:|for\s*\([^)]*;[^)]*;[^)]*\)|while\s*\([^)]*\)|try:|except\b[^\n]*:|with\b[^\n]*:|#include|public |private |protected |SELECT |INSERT |UPDATE |DELETE )/gi,
    ) || []
  ).length;
...
const tryExtractInlineCodeBlock = (
  text: string,
  index: number,
): { text: string; block: CodeBlock | null } => {
  const match = String(text || "").match(
    /([\s\S]*?)Code Example(?:\s*\(([^)]+)\))?\s*:\s*([\s\S]+)/i,
  );
  if (!match?.[3]) {
  ...
  const rawCode = stripCodeNarrativeTail(
    codeStart >= 0 ? remainder.slice(codeStart) : remainder,
  );
...
const tryExtractInlineCodeBlob = (
  text: string,
  index: number,
): { text: string; block: CodeBlock | null } => {
  const match = String(text || "").match(
    /([\s\S]*?)(\b(?:def\s+[A-Za-z_][A-Za-z0-9_]*\s*\(|class\s+[A-Za-z_][A-Za-z0-9_]*|function\s+[A-Za-z_][A-Za-z0-9_]*\s*\(|const\s+[A-Za-z_][A-Za-z0-9_]*\s*=|let\s+[A-Za-z_][A-Za-z0-9_]*\s*=|var\s+[A-Za-z_][A-Za-z0-9_]*\s*=|#include|public class|using\s+namespace|using\s+std::|fn\s+[A-Za-z_][A-Za-z0-9_]*\s*\(|SELECT |INSERT |UPDATE |DELETE |CREATE TABLE )[\s\S]*)/i,
  );
  if (!match?.[2]) {
    return { text, block: null };
  }
...
const normalizeMathExpressionOperators = (input: string): string =>
  String(input || "")
    ...
    .replace(
      /(\d(?:[\d.,]*))\s+(minus)\s+(\d(?:[\d.,]*))/gi,
      "$1 - $3",
    )
    .replace(
      /(\d(?:[\d.,]*))\s+(multiplied by|times)\s+(\d(?:[\d.,]*))/gi,
      "$1 * $3",
    )
    .replace(
      /(\d(?:[\d.,]*))\s+(divided by|over)\s+(\d(?:[\d.,]*))/gi,
      "$1 / $3",
    )
    .replace(
      /(\d(?:[\d.,]*))\s+(modulo|mod)\s+(\d(?:[\d.,]*))/gi,
      "$1 % $3",
    )
    .replace(
      /(\d(?:[\d.,]*))\s+(to the power of|raised to|power of)\s+(\d(?:[\d.,]*))/gi,
      "$1 ^ $3",
    )
    .replace(
      /\b([a-z])\s+(plus|minus|times|multiplied by|divided by|over|modulo|mod|to the power of|raised to|power of)\s+([a-z])\b/gi,
      (_match: string, left: string, opWord: string, right: string) => {
        const operator =
          /plus/i.test(opWord) ? "+"
          : /minus/i.test(opWord) ? "-"
...
const ensureBlankLineBetweenNumberedPoints = (lines: string[]): string[] => {
  const out: string[] = [];
...
const stripSpecialCharsFromPlainText = (line: string): string =>
  String(line || "")
...
const normalizeTableRowSpacing = (line: string): string => {
...
const polishPlainTextPunctuationLine = (line: string): string => {
  const trimmed = String(line || "").trim();
  if (!trimmed) return "";
  if (codePlaceholderPattern.test(trimmed)) return trimmed;
  if (tableDividerPattern.test(trimmed)) return trimmed;
  if (tableRowPattern.test(trimmed)) return normalizeTableRowSpacing(trimmed);
...
const extractCodeBlocks = (input: string): { text: string; blocks: CodeBlock[] } => {
  ...
  text = text.replace(markerCodeBlockPattern, (_match: string, code: string) => {
...
const rawRetrievalArtifactPattern =
  /\bdirect answer topic\b|\bprovide one specific output format\b|\bfinal polished answer\b|\bwikifunctions has a function related to this topic\b|\bwikidata\b|\bwikiquote\b|\bwikimedia commons\b|\bwiktionary\b|^\s*source\s*:\s*https?:\/\/\S+|\bhttps?:\/\/(?:[a-z]+\.)?wikipedia\.org\/[^\s<>"'`]+|\/\s*[a-z](?:[\s.]*[a-z]){3,20}\s*\//im;

const stripRawRetrievalArtifactsFromPlainText = (input: string): string => {
  const value = String(input || "").replace(/\r/g, "");
  if (!value) return value;

  const parts = value.split(/(```[\s\S]*?```)/g).filter(Boolean);
  const sanitizePlain = (segment: string): string => {
...
const splitDenseParagraphLines = (lines: string[]): string[] => {
  const out: string[] = [];
  for (const rawLine of lines) {
...
const splitDenseMathSolutionLines = (lines: string[]): string[] => {
  const out: string[] = [];
  ...
  for (const rawLine of lines) {
    ...
    const expanded = trimmed
      ...
      .replace(
        /,\s+(?=(?:Step\s*\d+|Calculation|Equation|Result|Final answer|Therefore|Hence|So|Then|Next)\b)/gi,
        "\n",
      )
      .replace(
        /,\s+(?=(?:[A-Za-z][A-Za-z0-9_]*\s*=\s*|[-(]?\d[\d.,()]*\s*[+\-*/%^x=]))/g,
        "\n",
      )
...
const convertLeadingNumberedBulletsToDashLines = (lines: string[]): string[] =>
  lines.map((rawLine) => {
    if (!rawLine) return rawLine;
    if (codePlaceholderPattern.test(rawLine.trim())) return rawLine;
...
const enforceDashListStyleOutsideCode = (text: string): string => {
  ...
  const normalizePlain = (segment: string): string =>
    convertLeadingNumberedBulletsToDashLines(String(segment || "").split("\n")).join("\n");
...
export const formatProfessionalReply = (input: string): string => {
  ...
  const normalizedLines = ensureBlankLineBetweenNumberedPoints(
    splitDenseMathSolutionLines(
      splitDenseParagraphLines(addParagraphSpacing(renumberLists(lines))),
    ),
  )
    .map((line) => {
      ...
      return polishPlainTextPunctuationLine(
        stripSpecialCharsFromPlainText(normalizeMathExpressionOperators(line)),
      );
    })
  ...
  const plainFallback = stripSpecialCharsFromPlainText(
    normalizeMathExpressionOperators(removeMarkdownArtifacts(stripRawRetrievalArtifactsFromPlainText(ascii))),
  ).trim();
  if (plainFallback) {
