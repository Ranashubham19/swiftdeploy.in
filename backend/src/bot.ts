...
import {
  ...
  isFreeOnlyApprovedModelId,
  MODEL_LIST,
  OPENROUTER_FREE_MODEL_ID,
} from "./openrouter/models.js";
import {
  currentEventsDisclaimer,
  buildIntentRoutingInstruction,
  classifyProfessionalIntent,
  detectIntent,
  mapProfessionalIntentToRuntimeIntent,
  normalizeIncomingUserMessage,
...
const CHAT_HISTORY_MAX_EXCHANGES = Math.max(
  15,
  Math.min(20, Number(process.env.CHAT_HISTORY_MAX_EXCHANGES || "20")),
);
...
const FAST_RECENT_CONTEXT_MESSAGES = Math.max(
  ...
  Math.min(
    ...
    Number(process.env.FAST_RECENT_CONTEXT_MESSAGES || "12"),
  ),
);
...
const REPLY_STICKER_PROBABILITY = Math.max(
  0,
  Math.min(1, Number(process.env.TG_STICKER_REPLY_PROBABILITY || "0")),
);
const PROFESSIONAL_MODEL_RECOVERY_MESSAGE =
  "Temporary model issue detected. Please resend the same query to retry the primary model route.";
...
const MAX_CONTINUATION_ROUNDS = Math.max(
  ...
  Math.min(
    4,
    Number(process.env.MAX_CONTINUATION_ROUNDS || (FORCE_OPENROUTER_FREE_ONLY_MODE ? "2" : "1")),
  ),
);
...
const MAX_MODEL_ATTEMPTS = Math.max(
  ...
  Math.min(
    6,
    Number(process.env.MAX_MODEL_ATTEMPTS || (FORCE_OPENROUTER_FREE_ONLY_MODE ? "4" : "1")),
  ),
);
const FINAL_SELF_CHECK_ENABLED =
  (process.env.FINAL_SELF_CHECK_ENABLED || "true").toLowerCase() !== "false";
const FINAL_SELF_CHECK_MIN_CHARS = Math.max(
  80,
  Math.min(1200, Number(process.env.FINAL_SELF_CHECK_MIN_CHARS || "180")),
);
const detailedPromptPattern =
  /\b(explain|detailed|detail|step by step|deep dive|comprehensive|teach|breakdown|why|how)\b/i;
const detailFollowUpOnlyPattern =
  /^(tell me in detail|explain in detail|in detail|detail|more detail|more details|tell me more|explain more|elaborate|deep dive|expand this|continue in detail|can you explain( it| this| that)? in detail|yes tell|yes explain)$/i;
...
const expandedResponsePattern =
  /\b(story|about|difference|compare|top|list|history|overview|explain|guide|full|complete)\b/i;
const TYPEWRITER_FALLBACK_ENABLED =
  (process.env.TYPEWRITER_FALLBACK_ENABLED || "false").toLowerCase() !== "false";
...
const TYPEWRITER_MAX_CHARS = Math.max(
  180,
  Math.min(1800, Number(process.env.TYPEWRITER_MAX_CHARS || "900")),
);
...
const CODE_REPAIR_ENABLED =
  (process.env.CODE_REPAIR_ENABLED || "true").toLowerCase() !== "false";
const CODE_REPAIR_MAX_TOKENS = Math.max(
  600,
  Math.min(
    3200,
    Number(process.env.CODE_REPAIR_MAX_TOKENS || (FORCE_OPENROUTER_FREE_ONLY_MODE ? "2200" : "1500")),
  ),
);
const TYPO_NORMALIZATION_ENABLED =
  (process.env.TYPO_NORMALIZATION_ENABLED || "true").toLowerCase() !== "false";
const SOFT_RATE_LIMIT_MODE =
  (process.env.SOFT_RATE_LIMIT_MODE || "false").toLowerCase() === "true";
...
const mediaDeepAnalysisPattern =
  /\b(full\s+description|describe|analysis|analyze|analyse|feedback|emotion|tone|mood|what\s+(?:was|is)\s+(?:said|told)|transcript|summari(?:ze|se)|improv(?:e|ement))\b/i;

const shouldRunDeepMediaAnalysis = (caption: string): boolean =>
  mediaDeepAnalysisPattern.test(String(caption || "").toLowerCase());

const mediaEnvelopePattern =
  /^\[(?:PHOTO|DOCUMENT|VOICE|AUDIO|VIDEO(?: NOTE| DOCUMENT)?|AUDIO DOCUMENT|LOCATION|CONTACT|STICKER)\s+MESSAGE\]/i;

const isMediaEnvelopePrompt = (prompt: string): boolean => {
...
const extractIntentSignalFromMediaPrompt = (prompt: string): string => {
  const raw = String(prompt || "");
  if (!isMediaEnvelopePrompt(raw)) return raw;
...
const buildMediaPromptForReply = (input: MediaPromptInput): string => {
...
const getPriorityChatReply = (input: string): string | null => {
  ...
  if (
    ...
    || /^(?:what(?:'s| is)\s+your\s+favou?rite\s+aston martin(?:\s+model)?|which\s+aston martin(?:\s+model)?\s+is\s+your\s+favou?rite)\??$/.test(
      text,
    )
  ...
  if (
    /^(?:what can you do|what do you do|what are your capabilities|what all can you do|tell me what you can do|can you tell me what you can do)\??$/.test(
      text,
    )
  ) {
  ...
  if (
    /^(?:are you ready to control my life|are you going to control my life|will you control my life|can you control my life)\??$/.test(
      text,
    )
...
const typoNormalizationRules: Array<{
  pattern: RegExp;
  replacement: string;
  label: string;
}> = [
...
const normalizeUserInput = (inputText: string): InputNormalizationResult => {
...
const definitionPromptPattern =
  /^(who(?:'s| is| was)|what(?:'s| is| was)|tell me about|explain|define|meaning of)\b/i;

const entityTokenCorrections: Record<string, string> = {
  epistein: "epstein",
  epstien: "epstein",
  epstine: "epstein",
  einstien: "einstein",
...
const LOCAL_ENTITY_FACTS: Record<string, string> = {
  hamster:
    "A hamster is a small rodent commonly kept as a pet, known for its short tail, cheek pouches, and nocturnal activity.",
  brabus:
  ...
  epstein:
    "Jeffrey Epstein was a U.S. financier convicted of sex offenses and died in custody in 2019 while awaiting trial on federal charges.",
  "jeffrey epstein":
    "Jeffrey Epstein was a U.S. financier convicted of sex offenses and died in custody in 2019 while awaiting trial on federal charges.",
  einstein:
    "Albert Einstein was a theoretical physicist best known for the theory of relativity and the equation E equals m c squared.",
  "albert einstein":
    "Albert Einstein was a theoretical physicist best known for the theory of relativity and the equation E equals m c squared.",
  joffrey:
    "Joffrey Baratheon is a fictional character in Game of Thrones, known as the cruel king of the Seven Kingdoms.",
  "joffrey baratheon":
    "Joffrey Baratheon is a fictional character in Game of Thrones, known as the cruel king of the Seven Kingdoms.",
  mississippi:
    "Mississippi is a state in the southeastern United States. If you meant the Mississippi River, it is one of the longest rivers in North America.",
  ps5:
    "The PS5 (PlayStation 5) is Sony's home video game console. It supports modern games, digital downloads, and features fast SSD storage with models including a standard edition and a digital edition.",
  "playstation 5":
    "The PlayStation 5 (PS5) is Sony's home video game console launched in 2020. It is known for fast load times, PS5-exclusive games, and support for backward-compatible PS4 titles.",
  psi:
    "PSI stands for pounds per square inch, a unit of pressure commonly used for tires, gas systems, and industrial pressure measurements.",
  "iphone 15":
    "iPhone 15 is a smartphone model from Apple released in the iPhone 15 lineup. If you want, I can explain its features, variants, launch pricing, or compare it with another phone.",
  palindrome:
    "A palindrome is a word, number, phrase, or sequence that reads the same backward and forward. Common examples include madam, racecar, and 121. In coding questions, palindrome checks are often used for strings, numbers, and interview practice problems.",
};
...
const extractDefinitionTopic = (inputText: string): string => {
  ...
  value = value
    .replace(
      /^(who(?:'s| is| was)|what(?:'s| is| was)|tell me about|explain|define|meaning of)\b[\s:,-]*/i,
      "",
    )
...
const lookupLocalEntityFact = (inputText: string): string | null => {
...
const looksLikeEntityMismatch = (inputText: string, answerText: string): boolean => {
  if (!definitionPromptPattern.test(String(inputText || "").trim())) {
    return false;
  }
  const topic = extractDefinitionTopic(inputText);
...
const looksLikeTemplateReuseMismatch = (inputText: string, answerText: string): boolean => {
...
const shouldGuardUnknownDefinitionPrompt = (inputText: string): boolean => {
  const raw = String(inputText || "").trim();
  if (!raw) return false;
  if (!definitionPromptPattern.test(raw)) return false;
  if (raw.length > 140) return false;

  const intent = detectIntent(raw);
  if (intent === "coding" || intent === "math" || intent === "current_events") return false;

  const topic = extractDefinitionTopic(raw);
  if (!topic) return false;
  if (LOCAL_ENTITY_FACTS[topic]) return false;
  ...
  if (risk === "medium" && (quotedTerm || ambiguousShape) && shortDefinition) return true;
  return false;
};

const getUnknownDefinitionClarificationReply = (inputText: string): string | null => {
  if (!shouldGuardUnknownDefinitionPrompt(inputText)) return null;
...
const getTypoAmbiguityClarificationReply = (inputText: string): string | null => {
  const raw = String(inputText || "").trim();
  if (!raw) return null;
...
const hasBalancedCodeFences = (text: string): boolean => {
...
const describeGenerationError = (error: unknown): string => {
  ...
  const PROFESSIONAL_MODEL_UNAVAILABLE_MESSAGE =
    "I could not reach the AI model right now due to temporary provider load. Please send your message again.";
...
const isFailureLikeOutput = (text: string): boolean => {
  const raw = String(text || "").trim();
  if (!raw) return true;
  ...
  const hasProviderFailureContext =
    /\b(openrouter|provider|model|quota|credits|authentication|network|service|fallback)\b/i.test(raw);
  const onlyWeakTransientWording =
    /\b(rate limit|please try again|try again in (a )?few seconds)\b/i.test(raw)
...
const normalizeLooseQualityText = (value: string): string =>
  String(value || "")
    .replace(/```[\s\S]*?```/g, " code ")
    .toLowerCase()
    .replace(/[^a-z0-9\s]/g, " ")
...
const containsPlaceholderCodeTemplate = (text: string): boolean =>
  /\breplace (?:this )?template with your requested logic\b/i.test(String(text || ""))
  || /\breplace with your requested logic\b/i.test(String(text || ""));

const containsTemplateCodeScaffoldSignals = (text: string): boolean => {
  const source = String(text || "").trim();
  if (!source) return false;

  const scaffoldPattern =
    /\b(replace (?:this )?template(?: with)?(?: your)? requested logic|replace with your requested logic|your logic here|implement (?:your|the) logic here|add your logic here|fill in (?:the )?(?:logic|implementation)|placeholder(?: code)?|todo)\b/i;
  const suspiciousReadyPattern =
    /\b(?:std::cout\s*<<\s*["']ready["']|console\.log\s*\(\s*["']ready["']\s*\)|print\s*\(\s*["']ready["']\s*\)|return\s+["']ready["']\s*;?)\b/i;
  const codeLikePattern =
    /```|#include\b|int\s+main\s*\(|std::cout|function\s+\w+\s*\(|console\.log|def\s+\w+\s*\(|class\s+\w+|public\s+class\b|return\s+/i;

  if (containsPlaceholderCodeTemplate(source)) return true;
  if (suspiciousReadyPattern.test(source) && codeLikePattern.test(source)) return true;
  return scaffoldPattern.test(source) && codeLikePattern.test(source);
};

const isLowValueDeflectionOutput = (text: string): boolean =>
  lowValueDeflectionOutputPattern.test(normalizeLooseQualityText(text))
  || containsPlaceholderCodeTemplate(text);

const hasBlockedTemplateLeak = (text: string): boolean =>
  blockedTemplateLeakPattern.test(normalizeLooseQualityText(text));

const isOpenRouterFreeLikeModelId = (modelId?: string | null): boolean =>
  isFreeOnlyApprovedModelId(modelId);

const stripAckAndPromptLabels = (value: string): string =>
  normalizeLooseQualityText(value)
    .replace(/^(ok|okay|sure|yes|alright|fine|well)\s+/i, "")
    .replace(/^(answer|question|prompt)\s+/i, "")
    .trim();

const isEchoLineCandidate = (normalizedPrompt: string, line: string): boolean => {
  ...
  const maxEchoLength = Math.max(
    normalizedPrompt.length + 40,
    Math.round(normalizedPrompt.length * 1.6),
  );
...
const stripLeadingPromptEchoLines = (prompt: string, reply: string): string => {
  const normalizedPrompt = normalizeLooseQualityText(prompt);
  if (!normalizedPrompt) return String(reply || "").trim();
...
const requestsPreviousFormatStyle = (value: string): boolean => {
...
const isMeaningfullyRepeatedReply = (previous: string, current: string): boolean => {
...
const findPreviousAssistantReplyForSamePrompt = (
...
const isDetailFollowUpOnlyPrompt = (value: string): boolean =>
  detailFollowUpOnlyPattern.test(normalizeComparableText(value));

const isContextualFollowUpPrompt = (value: string): boolean => {
  const normalized = normalizeComparableText(value);
  if (!normalized) return false;
  if (isMediaEnvelopePrompt(normalized) || isLikelyFileNameOnlyPrompt(normalized)) return false;
  if (parseContextReference(normalized).isReference) return true;
  if (detailFollowUpOnlyPattern.test(normalized)) return true;
  if (contextualFollowUpOnlyPattern.test(normalized)) return true;
  ...
  if (hasReference && hasAction) return true;

  return false;
};

type RecentConversationPair = {
  userPrompt: string;
  assistantReply: string;
};

const getConversationPairsFromRecentMessages = (
  recentMessages: OpenRouterMessage[],
): RecentConversationPair[] => {
  const pairs: RecentConversationPair[] = [];
...
const extractLatestContextFromRecentMessages = (
  recentMessages: OpenRouterMessage[],
  currentInput: string,
): { previousUserPrompt: string; previousAssistantReply: string } | null => {
  ...
  if (!previousUserPrompt) return null;
  return { previousUserPrompt, previousAssistantReply };
};

const buildDetailedFollowUpInput = (
  ...
  lines.push(
    "If the user later refers to the previous answer format/style, preserve the same structure while expanding the content.",
  );
  return lines.join("\n");
};

const buildContextualFollowUpInput = (
  previousUserPrompt: string,
  ...
  if (requestsPreviousFormatStyle(followUpMessage)) {
    lines.push(
      "Format/style rule: follow the previous answer's structure (for example steps, numbered list, headings, or table style) while answering this referenced topic.",
    );
  }
  return lines.join("\n");
};

const replaceLatestUserMessageForModel = (
  recentMessages: OpenRouterMessage[],
...
const buildRetryOnlyPoliteMessage = (
  ...
  if (localEntityFact) return localEntityFact;

  const capabilityLike =
    /\b(what can you do|what do you do|what are your capabilities|capabilities|can you code|do you know coding|languages do you support)\b/.test(
      lowered,
    )
  ...
  const codingImplementationLike =
    (/\b(write|generate|create|build|implement|debug|fix|refactor|optimize|convert|show|give|provide)\b/.test(
      lowered,
    )
      && /\b(code|script|function|class|api|endpoint|bot|query|algorithm|program|module|component|sql|regex|logic)\b/.test(
        lowered,
      ))
    || /\b(generate code|code for|implement this|implement that|write (?:a|an)?\s*(?:python|javascript|typescript|java|c\+\+|c#|go|rust|php|ruby|swift|kotlin)?\s*(?:script|program|function|class|api|bot))\b/.test(
      lowered,
    );
          ...
          : options?.intent === "math"
            ...
            : options?.intent === "current_events"
              ...
              : options?.complexGeneralRequest
                ? "smart"
                : "general"
    ) as keyof typeof CURATED_FREE_MODEL_POOLS;
...
const isCodeGenerationPrompt = (inputText: string): boolean => {
  ...
  const codingKeywordPattern =
    /\b(code+|cod|coding|program(?:ming)?|script|function|class|api|bot|algorithm|solution|snippet|query|sql|leetcode|dsa)\b/;
  const codingTypoPattern = /\b(codee+|codd?e|programing|snipet)\b/;
  const codingVerbPattern =
    /\b(write|generate|create|build|make|give|provide|implement|develop)\b/;
  const codingLanguagePattern =
    /\b(c\+\+|cpp|c#|csharp|python|javascript|typescript|java|go|golang|rust|php|ruby|swift|kotlin|sql|html|css|bash|shell|react|node(?:\.?js)?)\b/;
  const tableRequestWithoutCode =
    /\b(table|tabular|rows?\s+and\s+columns?|columns?\s+and\s+rows?)\b/.test(normalized)
  ...
  const wantsCode =
    codingVerbPattern.test(normalized)
    && (
...
const containsCodeSignals = (text: string): boolean => {
  const value = String(text || "");
  if (!value.trim()) return false;
  return (
    /Code Example(?:\s*\([^)]+\))?\s*:/i.test(value) ||
...
const isReadableCodeLayout = (code: string): boolean => {
...
const reflowSingleLineCode = (code: string, language: string): string => {
  ...
  if (language === "python") {
    const pythonFormatted = input
      .replace(/;\s*/g, "\n")
      .replace(
        /\s+(?=def |class |if |elif |else:|for |while |try:|except |finally:|with |return |import |from |pass|break|continue)/g,
        "\n",
      )
      .replace(
      ...
      .replace(
        /:\s+(?=[A-Za-z_][A-Za-z0-9_]*\s*=|if |for |while |return |pass|break|continue)/g,
        ":\n",
      )
      .replace(/\n{3,}/g, "\n\n")
      .trim();
  ...
  const genericFormatted = input
    ...
    .replace(/\s*\}/g, "\n}\n")
    .replace(
      /\s+(?=function |const |let |var |if |else |for |while |return |class |import |from )/g,
      "\n",
    )
    .replace(/\n{3,}/g, "\n\n")
    .trim();
...
const stripCodeExampleLabel = (value: string): string =>
  String(value || "").replace(/^\s*Code Example(?:\s*\([^)]+\))?\s*:\s*/i, "");

const stripTrailingNarrativeFromCode = (value: string): string =>
  String(value || "")
    .replace(
...
const extractCodeExplanation = (rawOutput: string): string => {
...
const normalizeExtractedCode = (code: string, language: string): string => {
  const normalized = stripTrailingNarrativeFromCode(
    stripCodeExampleLabel(String(code || "").replace(/\r\n/g, "\n")),
  ).trim();
...
const extractCodeArtifact = (rawOutput: string, prompt: string): CodeArtifact | null => {
  ...
  const codeExampleMatch = normalizedOutput.match(
    /Code Example(?:\s*\(([^)]+)\))?\s*:\s*([\s\S]+)/i,
  );
  if (codeExampleMatch?.[2]) {
    const languageHint = normalizeCodeLanguage(
      String(codeExampleMatch[1] || "").trim() || inferLanguageFromPrompt(prompt),
    );
  ...
  const mixedCodeStart = normalizedOutput.search(
    /\b(def |class |function |const |let |var |#include|public class|using |fn )/i,
  );
  if (mixedCodeStart >= 0) {
    const languageHint = normalizeCodeLanguage(inferLanguageFromPrompt(prompt));
    const extractedCode = normalizeExtractedCode(
      normalizedOutput.slice(mixedCodeStart),
      languageHint,
    );
    if (extractedCode && looksLikeCode(extractedCode)) {
      const extension = codeExtensionMap[languageHint] || "txt";
  ...
  if (looksLikeCode(rawOutput)) {
    const sanitizedInlineCode = stripTrailingNarrativeFromCode(
      stripCodeExampleLabel(rawOutput.trim()),
    );
...
const computeResponseTokenLimit = (
  ...
  const wantsExpandedResponse =
    expandedResponsePattern.test(inputText) || inputText.trim().length > 140;
  const isComplexPrompt =
    inputText.trim().length > 320
    || /\b(top\s+\d+|compare|comparison|difference|roadmap|guide|architecture|design|strategy|full|complete|end to end|comprehensive)\b/i.test(
      inputText,
    );
...
const isLikelyIncompleteText = (text: string): boolean => {
  const normalized = String(text || "").trim();
  if (!normalized || normalized.length < 120) return false;

  const lastChar = normalized.slice(-1);
  const hasTerminalPunctuation = /[.!?:)\]"]/u.test(lastChar);
  const trailingConnector = /\b(and|or|with|to|for|in|on|at|by|from|during|through|about|because|that|which)\s*$/i.test(
    normalized,
  );
...
const isComplexPromptNeedingCompleteAnswer = (prompt: string): boolean => {
  const text = String(prompt || "").trim();
  if (!text) return false;
  if (text.length > 320) return true;
  return /\b(explain|detailed|detail|deep dive|step by step|comprehensive|compare|comparison|difference|pros and cons|advantages|disadvantages|top\s+\d+|list|guide|roadmap|architecture|design|strategy|plan|full|complete|analysis)\b/i.test(
    text,
  );
};

const looksThinAnswerForComplexPrompt = (prompt: string, reply: string): boolean => {
  if (!isComplexPromptNeedingCompleteAnswer(prompt)) return false;
  const out = String(reply || "").trim();
  if (!out) return true;
  if (containsCodeSignals(out)) return false;
  ...
  if (out.length < 300 && Math.max(sentenceCount, lineCount) < 4) return true;
  return false;
};
...
export const buildBot = (options: BotBuildOptions): Telegraf<BotContext> => {
  ...
  const lastEngineStateByChat = new Map<number, {
    intent: Intent;
  ...
  const withRateLimit = async (ctx: BotContext): Promise<boolean> => {
    ...
    if (decision.allowed && !decision.softLimited) return true;
    if (decision.softLimited) {
      logger.warn(
        { key, resetAt: result.resetAt.toISOString() },
        "Soft rate-limit hit; continuing with best-effort response mode",
      );
      return true;
    }
    logger.warn(
      { key, resetAt: result.resetAt.toISOString(), retryAfterSeconds: decision.retryAfterSeconds },
      "Hard rate-limit enforced",
    );
    await ctx.reply(
      `Rate limit reached. Please wait about ${decision.retryAfterSeconds}s and try again.`,
    );
    return false;
  };
  ...
  const handleModelCommand = async (ctx: BotContext): Promise<void> => {
    ...
    if (args.length > 0) {
      ...
      await ctx.reply(
        `AI model is locked to ${getDisplayAiModelName()}. Current routing profile: ${updated.currentModel}`,
      );
      return;
    }
  ...
  const handleStyleCommand = async (ctx: BotContext): Promise<void> => {
    const chatInfo = await ensureChat(ctx);
    if (!chatInfo || !("text" in ctx.message!)) return;
    const text = ctx.message.text;
    ...
    const updated = await options.store.updateSettings(chatInfo.chat.id, {
      verbosity: mode as "concise" | "normal" | "detailed",
    });
    await ctx.reply(`Response style set to ${updated.verbosity}.`);
  };

  const handleEngineCommand = async (ctx: BotContext): Promise<void> => {
    const chatInfo = await ensureChat(ctx);
    if (!chatInfo || !("text" in ctx.message!)) return;
    const text = ctx.message.text;
    ...
    const wantsJson =
      (sub === "status" && (args[1] || "").toLowerCase() === "json")
      || sub === "json";
    ...
    if (sub === "json" || sub === "status") {
      ...
      const payload = {
        ...
        features: {
          ...
          codeExecutionRepairLoop:
            String(process.env.ADVANCED_CODE_EXECUTION_FIX_ENABLED || "").toLowerCase() !== "false",
    ...
    if (isCheck) {
      ...
      const effectiveInput = followUpContext
        ? (detailOnlyFollowUp
          ? buildDetailedFollowUpInput(
            followUpContext.previousUserPrompt,
            followUpContext.previousAssistantReply,
          )
          : buildContextualFollowUpInput(
            followUpContext.previousUserPrompt,
            interpretedInput,
            followUpContext.previousAssistantReply,
          ))
  ...
  const generateReply = async (
    ...
    await options.lockManager.withChatLock(chatInfo.chat.id, async () => {
      ...
      const interpretedIntentSignal = mediaEnvelopeInterpreted
        ...
        : interpretedInput;
      let professionalIntent: ProfessionalIntent = forceVision
        ? "technical_question"
        : classifyProfessionalIntent(interpretedIntentSignal || interpretedInput, false);
      let intent: Intent = forceVision
        ? "general"
        : mapProfessionalIntentToRuntimeIntent(
          professionalIntent,
          interpretedIntentSignal || interpretedInput,
        );

      const typingStop = startTypingIndicator(ctx);
      const placeholder = await ctx.reply("Thinking...");
      const detailedRequest =
        detailedPromptPattern.test(interpretedInput) || interpretedInput.length > 350;
      ...
      if (!forceVision) {
        professionalIntent = classifyProfessionalIntent(
          effectiveIntentSignal || effectiveInput,
          recentMessages.length > 0,
        );
        if (followUpContext) {
          professionalIntent = "follow_up";
        }
        intent = mapProfessionalIntentToRuntimeIntent(
          professionalIntent,
          effectiveIntentSignal || effectiveInput,
        );
      }
      ...
      const previousSamePromptReply = findPreviousAssistantReplyForSamePrompt(
        recentMessages,
        effectiveInput,
      );
      ...
      const codeGenerationRequest =
        professionalIntent === "coding_request" && isCodeGenerationPrompt(effectiveInput);
      const complexProfessionalRequest = isComplexPromptNeedingCompleteAnswer(
        effectiveInput,
      );
      ...
      if (isOpenRouterFreeLikeModelId(route.modelId)) {
        route.temperature = Math.min(
          route.temperature,
          codeGenerationRequest ? 0.12 : intent === "math" ? 0.1 : complexProfessionalRequest ? 0.18 : 0.25,
        );
      ...
      if (previousSamePromptReplyPreview) {
        messages.push({
          role: "system",
          content:
            `Repeat-question rule: the user asked a similar/same question again. Provide a fresh, improved professional answer and do not repeat previous wording.\nPrevious answer preview (avoid repeating this wording): ${previousSamePromptReplyPreview}`,
        });
      }
      ...
      let messagesForFinal: OpenRouterMessage[] =
        forceVision && imageUrls.length > 0
      ...
      try {
        ...
        if (!stopped && outputBuffer.trim()) {
          const shouldContinue =
            wasTruncatedByTokens || isLikelyIncompleteText(outputBuffer);
          for (
            let round = 0;
            shouldContinue && round < continuationRoundsForRequest;
            round += 1
          ) {
        ...
        if (!stopped && !outputBuffer.trim()) {
          logger.warn(
            { chatId: chatInfo.chat.id, model: activeModelId },
            "Primary generation returned empty output; attempting emergency completion",
          );

          const emergencyResult = await callWithFallback((modelId) =>
            options.openRouter.chatCompletion(
              {
                model: modelId,
                messages: messagesForFinal,
        ...
        let extractedForQuality = outputBuffer.trim()
          ...
          : null;
        const forceFreeModelCodeRepairPass =
          codeGenerationRequest
          && isOpenRouterFreeLikeModelId(activeModelId);
        const needsCodeRepair =
          codeGenerationRequest &&
          CODE_REPAIR_ENABLED &&
          outputBuffer.trim() &&
          (
        ...
        if (needsCodeRepair) {
          const preferredCodeModelId =
            MODEL_LIST.find((model) => model.key === "code")?.id || route.modelId;
          const previousModelId = activeModelId;

          logger.info(
            { chatId: chatInfo.chat.id, primaryModel: previousModelId, repairModel: preferredCodeModelId },
            "Running targeted code repair/finalization pass",
          );

          try {
            activeModelId = preferredCodeModelId;
            const repairedCodeResult = await callWithFallback((modelId) =>
              options.openRouter.chatCompletion(
                {
                  model: modelId,
                  messages: [
...
- Do not return partial code or placeholders.`,
                  ...
                  temperature: 0.1,
                  max_tokens: Math.min(
                    CODE_REPAIR_MAX_TOKENS,
                    Math.max(responseTokenLimit, 900),
                  ),
                },
                { signal: controller.signal },
              ),
      ...
      if (canRunRecoveryPass) {
        try {
          const recovery = await callWithFallback((modelId) =>
            options.openRouter.chatCompletion(
              {
                model: modelId,
                messages: [
                  {
                    role: "system",
                    content:
                      "Recovery mode: infer the user's intent even if there are spelling mistakes. Answer directly with best effort. Do not ask for more context unless the request is empty or unsafe.",
                  },
                  ...messagesForFinal,
                ],
      ...
      const retryOnlyFallback = buildRetryOnlyPoliteMessage(
        fallbackReferenceInput,
        professionalIntent,
      );
      ...
      const cleanedRawModelOutput = stripLeadingPromptEchoLines(
        effectiveInput,
        rawModelOutput,
      ) || rawModelOutput;
      const rawOutputHasTemplateCodeScaffold =
        codeGenerationRequest && containsTemplateCodeScaffoldSignals(rawModelOutput);
      ...
      if (!outputBuffer.trim()) {
        outputBuffer = formatProfessionalReply(
          retryOnlyFallback || displayOutput || effectiveInput,
        );
      }

      const forceAdvancedRewriteForFreeModel =
        !stopped &&
        !generationErrored &&
        !codeArtifact &&
        isOpenRouterFreeLikeModelId(activeModelId) &&
      ...
      if (needsQualityRewrite) {
        ...
        if (complexProfessionalRequest) {
          rewritePromptLines.push(
            "- This is a complex/big question. Give a complete professional answer with clear sections and enough detail.",
          );
        }
        if (forceAdvancedRewriteForFreeModel) {
          rewritePromptLines.push(
            "- Free-model quality mode is active. Improve logic, completeness, and professionalism before finalizing.",
          );
        }
        if (requestsPreviousFormatStyle(interpretedInput) || requestsPreviousFormatStyle(effectiveInput)) {
          rewritePromptLines.push(
            "- The user referred to the previous answer format/style. Preserve the same structure while improving the answer quality.",
          );
        }
        if (previousSamePromptReplyPreview) {
        ...
        try {
          const rewritten = await callWithFallback((modelId) =>
            options.openRouter.chatCompletion(
              {
                model: modelId,
                messages: [
          ...
          if (rewritten.content.trim()) {
            ...
            const stillWeak =
              isPromptEchoLikeReply(effectiveInput, rewrittenFormatted)
          ...
          logger.warn(
            {
              ...
              model: activeModelId,
              error:
                qualityRewriteError instanceof Error
      ...
      const shouldRunFinalSelfCheckPass = (candidate: string): boolean => {
        if (!FINAL_SELF_CHECK_ENABLED) return false;
        const text = String(candidate || "").trim();
        if (!text) return false;
        if (text.length < FINAL_SELF_CHECK_MIN_CHARS && !codeGenerationRequest) return false;
        if (codeGenerationRequest) return true;
        if (isLikelyIncompleteText(text)) return true;
        if (complexProfessionalRequest && looksThinAnswerForComplexPrompt(effectiveInput, text)) return true;
      ...
      if (!stopped && !generationErrored && shouldRunFinalSelfCheckPass(outputBuffer)) {
        ...
        try {
          const verified = await callWithFallback((modelId) =>
            options.openRouter.chatCompletion(
              {
                model: modelId,
                messages: [
          ...
          if (verified.content.trim()) {
            ...
            const stillBad =
              isPromptEchoLikeReply(effectiveInput, verifiedFormatted)
      ...
      const finalReplyStillWeak =
        (codeGenerationRequest && containsTemplateCodeScaffoldSignals(rawModelOutput || outputBuffer))
        || (
          !codeArtifact &&
          (
            isPromptEchoLikeReply(effectiveInput, outputBuffer)
      ...
      if (finalReplyStillWeak) {
        outputBuffer = formatProfessionalReply(
          buildRetryOnlyPoliteMessage(effectiveInput, professionalIntent),
        );
      }
      ...
      const shouldUseTypewriterFallback =
        TYPEWRITER_FALLBACK_ENABLED &&
        !sawLiveDelta &&
        !codeArtifact &&
  ...
  bot.on("text", async (ctx) => {
    ...
    try {
      ...
      const fallbackOutput = formatProfessionalReply(
        buildRetryOnlyPoliteMessage(ctx.message.text, fallbackIntent),
      );
  ...
  bot.on("photo", async (ctx) => {
    try {
      ...
      const largestPhoto = [...(ctx.message.photo || [])].sort(
  ...
  bot.on("audio", async (ctx) => {
    try {
      ...
      logger.error(
        { error: error instanceof Error ? error.stack : String(error) },
        "Failed to handle audio message",
      );
  ...
  bot.on("video", async (ctx) => {
    try {
      ...
      logger.error(
        { error: error instanceof Error ? error.stack : String(error) },
        "Failed to handle video message",
      );
  ...
  bot.on("document", async (ctx) => {
    try {
      ...
      logger.error(
        { error: error instanceof Error ? error.stack : String(error) },
        "Failed to handle document message",
      );
