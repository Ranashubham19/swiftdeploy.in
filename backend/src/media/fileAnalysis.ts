...
type PhotoPromptInput = DocumentPromptInput;
...
type OcrSpaceResult = {
  text: string | null;
  error: string | null;
  method: "url" | "upload" | "none";
};

const FILE_DOWNLOAD_TIMEOUT_MS = Math.max(
  5000,
  Math.min(25000, Number(process.env.FILE_DOWNLOAD_TIMEOUT_MS || "15000")),
);
const FILE_MAX_BYTES = Math.max(
  1024 * 1024,
  Math.min(30 * 1024 * 1024, Number(process.env.FILE_MAX_BYTES || `${12 * 1024 * 1024}`)),
);
const FILE_CONTENT_PROMPT_CHARS = Math.max(
  1200,
  Math.min(9000, Number(process.env.FILE_CONTENT_PROMPT_CHARS || "3600")),
);
const FILE_VISION_PROMPT_CHARS = Math.max(
  300,
  Math.min(2200, Number(process.env.FILE_VISION_PROMPT_CHARS || "900")),
);
const requireFromMedia = createRequire(import.meta.url);
type PdfParseV1Fn = (dataBuffer: Buffer) => Promise<{ text?: string }>;
type PdfParseV2Instance = {
  getText?: (options?: Record<string, unknown>) => Promise<{ text?: string } | string>;
  destroy?: () => Promise<void> | void;
};
type PdfParseV2Ctor = new (options: { data?: Buffer | Uint8Array | ArrayBuffer; url?: string }) => PdfParseV2Instance;
type PdfParseAdapter = {
  v1Fn?: PdfParseV1Fn;
  v2Ctor?: PdfParseV2Ctor;
};
let cachedPdfParseAdapter: PdfParseAdapter | null = null;
...
const looksImageLike = (mimeType: string, extension: string): boolean => {
  if (String(mimeType || "").toLowerCase().startsWith("image/")) return true;
  return imageExtensions.has(String(extension || "").toLowerCase());
};

const looksPdfLike = (mimeType: string, extension: string): boolean => {
  return String(mimeType || "").toLowerCase() === "application/pdf" || String(extension || "").toLowerCase() === "pdf";
};

const looksTextLike = (mimeType: string, extension: string): boolean => {
  const mime = String(mimeType || "").toLowerCase();
  if (textLikeMimeHints.some((hint) => mime.startsWith(hint) || mime === hint)) return true;
  return textExtensions.has(String(extension || "").toLowerCase());
};

const deriveOcrFileTypeHint = (mimeType: string, extension: string): string => {
  const ext = String(extension || "").trim().toLowerCase();
  if (ext === "pdf" || String(mimeType || "").toLowerCase() === "application/pdf") return "PDF";
...
const stripHtmlLikeTags = (html: string): string => {
...
const scoreTextLikelihood = (text: string): number => {
...
const decodeBufferAsText = (buffer: Buffer, extension: string): string | null => {
  ...
  if (!forceText && textScore < 0.76) return null;
...
type ZipDecodedEntry = {
  name: string;
  data: Buffer;
};

const findZipEocdOffset = (buffer: Buffer): number => {
...
const parseZipEntries = (buffer: Buffer): ZipDecodedEntry[] => {
  ...
  if (centralDirOffset <= 0 || centralDirOffset >= buffer.length) return [];
  ...
  while (offset + 46 <= centralDirEnd) {
    ...
    if (centralSig !== 0x02014b50) break;
...
const extractTextFromOfficeXmlZip = (buffer: Buffer, extension: string): string | null => {
  if (!buffer || buffer.length === 0) return null;
  const ext = String(extension || "").toLowerCase().trim();
  if (!officeXmlExtensions.has(ext)) return null;
...
const normalizePdfFragment = (value: string): string =>
  normalizePlainText(
    String(value || "")
...
const looksHumanReadablePdfFragment = (value: string): boolean => {
  ...
  if (alphaRatio < 0.22 && digits < 3) return false;
  if (symbolRatio > 0.28) return false;
  return true;
};

const isLikelyGibberishText = (value: string): boolean => {
  ...
  if (vowelWordRatio < 0.42) return true;
  if (tokenDensity < 0.45 && digitChars > alphaChars) return true;
  return false;
};

const extractTextFromPdfRawBuffer = (buffer: Buffer): string | null => {
  if (!buffer || buffer.length === 0) return null;
  ...
  for (const token of matches.slice(0, 1200)) {
    ...
    if (!decoded) continue;
    if (!looksHumanReadablePdfFragment(decoded)) continue;
  ...
  if (isLikelyGibberishText(joined)) return null;
  return joined;
};

const trimForPrompt = (text: string, maxChars: number): { value: string; truncated: boolean } => {
...
const normalizeVisionChatEndpoint = (raw: string, fallback: string): string => {
...
const getPdfParseAdapter = (): PdfParseAdapter | null => {
  if (cachedPdfParseAdapter) return cachedPdfParseAdapter;
  try {
    const loaded = requireFromMedia("pdf-parse");
    const candidate = loaded?.default || loaded;
    const adapter: PdfParseAdapter = {};

    if (typeof candidate === "function") {
      adapter.v1Fn = candidate as PdfParseV1Fn;
    }
    const v2CtorCandidate = loaded?.PDFParse || candidate?.PDFParse;
    if (typeof v2CtorCandidate === "function") {
      adapter.v2Ctor = v2CtorCandidate as PdfParseV2Ctor;
    }
    if (!adapter.v1Fn && !adapter.v2Ctor) return null;
...
const extractTextFromPdfLibrary = async (buffer: Buffer): Promise<string | null> => {
  if (!buffer || buffer.length === 0) return null;
  ...
  if (adapter.v1Fn) {
    try {
      ...
      if (text && !isLikelyGibberishText(text)) return text;
    } catch {
  ...
  if (adapter.v2Ctor) {
    let instance: PdfParseV2Instance | null = null;
    try {
...
const extractParsedTextFromOcrSpacePayload = (payload: any): string | null => {
  const parsed = Array.isArray(payload?.ParsedResults) ? payload.ParsedResults : [];
  const text = parsed
    .map((entry: any) => String(entry?.ParsedText || "").trim())
    .filter(Boolean)
    .join("\n")
    .trim();
  return text || null;
...
const extractOcrSpaceError = (payload: any): string | null => {
  ...
  if (base) return base;
  if (payload?.IsErroredOnProcessing) return "OCR provider returned processing error.";
  return null;
};

const extractTextViaOcrSpace = async (
  input: {
    fileUrl?: string;
  ...
  if (!apiKey) return { text: null, error: "OCR API key is not configured.", method: "none" };
  ...
  if (/^https?:\/\//i.test(fileUrl)) {
    ...
    try {
      ...
      if (fileTypeHint) body.set("filetype", fileTypeHint);
      ...
      if (response.ok) {
        ...
        if (text) return { text, error: null, method: "url" };
        const err = extractOcrSpaceError(payload);
        if (err) bestError = err;
      } else {
  ...
  try {
    ...
    if (fileTypeHint) form.set("filetype", fileTypeHint);
    ...
    const payload = (await response.json().catch(() => ({}))) as any;
    const text = extractParsedTextFromOcrSpacePayload(payload);
    if (text) return { text, error: null, method: "upload" };
    return {
      text: null,
...
const analyzeImageViaOpenRouter = async (
  imageUrl: string,
  caption: string,
): Promise<string | null> => {
  ...
  const featureToggle = String(
    process.env.LEGACY_IMAGE_ANALYSIS_ENABLED || process.env.VISION_ANALYSIS_ENABLED || "",
  ).trim().toLowerCase();
  const explicitlyDisabled = ["0", "false", "off", "no"].includes(featureToggle);

  const providers = [
    {
      endpoint: normalizeVisionChatEndpoint(
        String(process.env.NVIDIA_VISION_BASE_URL || process.env.NVIDIA_BASE_URL || "").trim(),
  ...
  if (explicitlyDisabled || providers.length === 0) return null;
...
const downloadFileBuffer = async (
  fileUrl: string,
): Promise<{ buffer: Buffer | null; notes: string[] }> => {
  const notes: string[] = [];
...
const analyzeDocumentFile = async (input: DocumentPromptInput): Promise<DocumentAnalysis> => {
  const fileUrl = String(input.fileUrl || "").trim();
  ...
  const category: DocumentAnalysis["category"] = looksImageLike(mimeType, extension)
    ? "image"
  ...
  if (category === "image") {
    ...
    if (!vision) notes.push("Vision summary was unavailable.");
    return {
      category,
...
const buildDocumentResponseGuide = (
  caption: string,
  hasExtractedContent: boolean,
  extractionStatus: DocumentAnalysis["extractionStatus"],
): string => {
...
export const buildPhotoPromptFromTelegramFile = async (
...
export const buildDocumentPromptFromTelegramFile = async (
  input: DocumentPromptInput,
): Promise<string> => {
