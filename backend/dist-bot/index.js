...
const OPENROUTER_BASE_URL = (process.env.NVIDIA_BASE_URL || process.env.OPENROUTER_BASE_URL || "").trim()
...
const TELEGRAM_RUNTIME_SOURCE = (process.env.TELEGRAM_RUNTIME_SOURCE ||
    "legacy")
    .trim()
    .toLowerCase();
const ENABLE_STANDALONE_TELEGRAM_RUNTIME = TELEGRAM_RUNTIME_SOURCE === "standalone";
...
const SESSION_SECRET = SESSION_SECRET_FROM_ENV || "dev_session_secret_change_me_please_32_chars";
const TELEGRAM_WEBHOOK_SECRET = (process.env.TELEGRAM_WEBHOOK_SECRET || "").trim();
const PORT = Number(process.env.PORT || 4000);
const MAX_INPUT_CHARS = Math.max(1000, Number(process.env.MAX_INPUT_CHARS || 12000));
const MAX_OUTPUT_TOKENS = Math.max(300, Number(process.env.MAX_OUTPUT_TOKENS || 1800));
const STREAM_EDIT_INTERVAL_MS = Math.max(80, Math.min(260, Number(process.env.STREAM_EDIT_INTERVAL_MS || 180)));
const OPENROUTER_TIMEOUT_MS = Math.max(8000, Number(process.env.OPENROUTER_TIMEOUT_MS || 14000));
const OPENROUTER_MAX_RETRIES = Math.max(0, Math.min(4, Number(process.env.OPENROUTER_MAX_RETRIES || 1)));
const OPENROUTER_RETRY_BASE_DELAY_MS = Math.max(50, Number(process.env.OPENROUTER_RETRY_BASE_DELAY_MS || 180));
if (IS_PRODUCTION &&
    ENABLE_STANDALONE_TELEGRAM_RUNTIME &&
    TELEGRAM_USE_WEBHOOK &&
    !TELEGRAM_WEBHOOK_SECRET) {
    throw new Error("TELEGRAM_WEBHOOK_SECRET is required in production when standalone Telegram runtime uses webhooks");
}
...
const verifyTelegramWebhookRequest = (req) => {
    return verifyTelegramWebhookSecretHeader(String(req.headers["x-telegram-bot-api-secret-token"] || ""), TELEGRAM_WEBHOOK_SECRET, { isProduction: IS_PRODUCTION, webhookEnabled: TELEGRAM_USE_WEBHOOK });
};
...
app.post("/webhook", async (req, res) => {
    ...
    try {
        ...
        logger.error({ error: error instanceof Error ? error.stack : String(error) }, "Webhook handler failed");
        if (!res.headersSent) {
...
const server = app.listen(PORT, "0.0.0.0", async () => {
    ...
    const startTelegramRuntime = async () => {
        ...
        try {
            ...
            logger.warn({ error: error instanceof Error ? error.stack : String(error) }, "setMyCommands failed; continuing startup");
        }
        try {
            if (TELEGRAM_USE_WEBHOOK) {
                if (!APP_URL) {
                    logger.warn("TELEGRAM_USE_WEBHOOK=true but APP_URL is missing; falling back to long-polling");
            ...
            startupAttempts = 0;
            logger.info({ username: me.username, id: me.id }, "Telegram runtime ready");
        }
        catch (error) {
    ...
    if (!ENABLE_STANDALONE_TELEGRAM_RUNTIME) {
        logger.warn({ TELEGRAM_RUNTIME_SOURCE }, "Standalone Telegram runtime disabled to avoid dual-runtime conflicts; website backend/server.ts remains source of truth");
        return;
    }
    void startTelegramRuntime();
});
