...
export const getIsolatedRunnerConfig = () => {
    ...
    const required = (process.env.ISOLATED_CODE_RUNNER_REQUIRED || "false").toLowerCase() === "true";
    return { url, apiKey, timeoutMs, required };
};
export const isIsolatedRunnerConfigured = () => Boolean(getIsolatedRunnerConfig().url);
export const normalizeIsolatedRunnerResponse = (data, language) => ({
    ok: Boolean(data?.ok),
    language: clean(data?.language) || language,
    command: clean(data?.command),
    args: Array.isArray(data?.args) ? data.args.map((v) => String(v)) : [],
    exitCode: typeof data?.exitCode === "number" && Number.isFinite(data.exitCode)
        ? data.exitCode
        : null,
...
export const executeCodeViaIsolatedRunner = async (payload) => {
