...
const CODE_EXECUTION_ENABLED = (process.env.ADVANCED_CODE_EXECUTION_ENABLED || "false").toLowerCase() === "true";
export const isSubprocessCodeExecutionEnabled = () => CODE_EXECUTION_ENABLED;
...
const MAX_CODE_BYTES = Math.max(256, Math.min(80_000, Number(process.env.CODE_EXEC_MAX_BYTES || 20_000)));
const normalizeOutput = (value) => String(value || "").replace(/\0/g, "").slice(0, 12_000);
const runProcess = (command, args, opts) => new Promise((resolve) => {
...
const choosePythonCommand = async (scriptPath, timeoutMs) => {
...
export const executeCodeWithSubprocess = async (code, languageHint, opts) => {
...
export const executeCodeWithExecutionLayer = async (code, languageHint, opts) => {
