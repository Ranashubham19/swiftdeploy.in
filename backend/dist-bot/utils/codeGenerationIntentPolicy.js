const isGenericCodingIntentPrompt = (text) => {
    const v = String(text || '').trim().toLowerCase().replace(/\s+/g, ' ');
    if (!v)
        return false;
    const shortGenericCoding = v.split(' ').length <= 5
        && /\b(coding|code)\b/.test(v)
        && !/\b(write|implement|debug|fix|solve|function|class|algorithm|leetcode|in\s+\w+|for\s+\w+)\b/.test(v);
    if (shortGenericCoding)
        return true;
    if (/^(coding|code)\b/.test(v) && v.split(' ').length <= 3)
        return true;
    if (/^(ok\s+)?i (want|need|like)\s+(coding|code)\b/.test(v) && v.split(' ').length <= 8)
        return true;
    if (/^(know|learn|teach me)\s+(coding|code)\b/.test(v) && v.split(' ').length <= 8)
        return true;
    if (/^(can you|could you|do you)\s+(help|support)\s+(with\s+)?coding\b/.test(v) && v.split(' ').length <= 10)
        return true;
    return false;
};
const lineLooksLikeCodeSignal = (line) => {
...
const containsCodeLikeSignals = (text) => {
...
const stripCodeArtifactsFromNarrative = (input) => {
...
export const isExplicitCodeGenerationRequest = (prompt) => {
    const value = String(prompt || '').toLowerCase().replace(/\s+/g, ' ').trim();
    if (!value || isGenericCodingIntentPrompt(value))
        return false;
    if (/\b(write code|show example|generate script|create implementation)\b/.test(value))
        return true;
    if (/\b(with code|in code|code only)\b/.test(value))
        return true;
    if (/\b(write|show|generate|create|implement|build)\b[\s\S]{0,90}\b(code|script|implementation|solution|program|snippet|function|class|query|regex)\b/.test(value)) {
        return true;
    }
    if (/\b(give|provide|share|return|send)\b[\s\S]{0,90}\b(code|script|implementation|solution|snippet|program)\b/.test(value)) {
        return true;
    }
    if (/\b(solve|implement|build|write)\b[\s\S]{0,80}\b(in|using)\s+(python|javascript|typescript|java|c\+\+|cpp|c#|csharp|go|golang|rust|sql|php|ruby|swift|kotlin)\b/.test(value)) {
        return true;
    }
    if (/\b(debug|fix|refactor|optimize|correct)\b[\s\S]{0,60}\b(code|script|function|query|program)\b/.test(value))
        return true;
...
export const enforceCodeGenerationIntentPolicy = (prompt, reply, options) => {
    const source = String(reply || '').trim();
    if (!source)
        return source;
    if (isExplicitCodeGenerationRequest(prompt))
        return source;
