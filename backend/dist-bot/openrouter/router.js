import { FALLBACK_MODEL_ID, FORCE_OPENROUTER_FREE_ONLY_MODE, MODEL_REGISTRY, resolveModelFromKey, } from "./models.js";
const PROFESSIONAL_INTENT_LABELS = {
    greeting: "Greeting",
...
const INCOMING_SHORTHAND_FIXES = [
...
const fixTypos = (input) => String(input || "").replace(/\b[a-z][a-z0-9_-]*\b/gi, (token) => {
    const corrected = TYPO_CORRECTIONS[token.toLowerCase()];
    return corrected || token;
});
export const normalizeIncomingUserMessage = (text) => {
...
const isGreetingPrompt = (text) => /^(hi|hello|hey|hii|yo|good morning|good afternoon|good evening)\b/i.test(text);
const isCasualConversationPrompt = (text) => /^(thanks|thank you|ok|okay|cool|nice|great|awesome|lol|haha|hmm|hmmm|yo|sup|good night|bye|see you)\b/i.test(text);
const isClarificationPrompt = (text) => /\b(clarify|what do you mean|which one|can you clarify|please clarify|explain that part|did you mean|what exactly|more clear)\b/i.test(text);
const isOpinionRequestPrompt = (text) => /\b(what do you think|your opinion|do you think|is it better|which is better|recommend|should i|would you choose|pros and cons)\b/i.test(text);
const isProblemSolvingPrompt = (text) => /\b(error|issue|problem|not working|does not work|doesn't work|fails?|failing|failed|crash|stuck|cannot|can't|unable to|troubleshoot|fix this|why is this broken)\b/i.test(text);
const isCapabilityQuestionPrompt = (text) => {
...
const isCodingImplementationRequest = (text) => {
    const value = String(text || "").toLowerCase().replace(/\s+/g, " ").trim();
    if (!value || isCapabilityQuestionPrompt(value))
        return false;
    const explicitRequest = /\b(write|generate|create|build|make|implement|debug|fix|refactor|optimize|convert|show|give|provide)\b/.test(value)
        && /\b(code|script|function|class|api|endpoint|bot|query|algorithm|program|module|component|sql|regex|logic)\b/.test(value);
    const implementationPhrase = /\b(generate code|write (?:a|an)?\s*(python|javascript|typescript|java|c\+\+|c#|go|rust|php|ruby|swift|kotlin)?\s*(script|program|function|class|api|bot)|create implementation|code for|implement this|implement that)\b/.test(value);
...
const isMathLikePrompt = (text) => /\b(math|algebra|calculus|equation|differentiate|integrate|solve|probability|statistics|matrix|derive)\b|[0-9]+\s*[\+\-*/]\s*[0-9]+/i.test(text);
const isCurrentEventsPrompt = (text) => /\b(news|latest|today|yesterday|breaking|current events|what happened|stock today|election today|as of)\b/i.test(text);
const isFollowUpReferencePrompt = (text) => {
    const value = String(text || "").toLowerCase().replace(/\s+/g, " ").trim();
    if (!value)
        return false;
...
const isTechnicalQuestionPrompt = (text) => {
    const value = String(text || "").toLowerCase().replace(/\s+/g, " ").trim();
    if (!value)
        return false;
    const technicalSignals = /\b(api|database|server|backend|frontend|docker|kubernetes|cloud|linux|network|http|https|oauth|jwt|cache|queue|microservice|architecture|typescript|javascript|python|java|c\+\+|sql|redis|postgres|mongodb|deployment|devops)\b/.test(value);
...
const looksUnclearPrompt = (text, hasHistory) => {
    const value = String(text || "").toLowerCase().replace(/\s+/g, " ").trim();
    if (!value)
        return true;
    if (isGreetingPrompt(value) || isCasualConversationPrompt(value))
        return false;
    if (isCapabilityQuestionPrompt(value) || isCodingImplementationRequest(value))
        return false;
    if (isProblemSolvingPrompt(value) || isOpinionRequestPrompt(value) || isClarificationPrompt(value))
        return false;
    if (isMathLikePrompt(value) || isCurrentEventsPrompt(value) || isTechnicalQuestionPrompt(value))
        return false;
...
export const classifyProfessionalIntent = (text, hasHistory = false) => {
    const normalized = normalizeIncomingUserMessage(text);
    const value = normalized.normalizedText || normalized.loweredText;
    if (!value)
        return "unclear";
    if (hasHistory && isFollowUpReferencePrompt(value))
        return "follow_up";
    if (isGreetingPrompt(value))
        return "greeting";
    if (isCapabilityQuestionPrompt(value))
        return "capability_question";
    if (isCodingImplementationRequest(value))
        return "coding_request";
    if (isProblemSolvingPrompt(value))
        return "problem_solving";
    if (isClarificationPrompt(value))
        return "clarification";
    if (isOpinionRequestPrompt(value))
        return "opinion_request";
    if (isTechnicalQuestionPrompt(value))
        return "technical_question";
    if (isCasualConversationPrompt(value))
        return "casual_conversation";
    if (looksUnclearPrompt(value, hasHistory))
        return "unclear";
    return "casual_conversation";
};
export const mapProfessionalIntentToRuntimeIntent = (intent, text) => {
    const value = String(text || "").toLowerCase();
    if (isMathLikePrompt(value))
        return "math";
    if (isCurrentEventsPrompt(value))
        return "current_events";
    if (intent === "coding_request")
        return "coding";
    return "general";
};
export const detectIntent = (text) => mapProfessionalIntentToRuntimeIntent(classifyProfessionalIntent(text, false), text);
export const buildIntentRoutingInstruction = (intent) => {
...
export const routeModel = (selectedModelKey, intent) => {
...
export const currentEventsDisclaimer = "I do not have live web browsing in this setup. I can still help with background context and likely scenarios based on known information.";
